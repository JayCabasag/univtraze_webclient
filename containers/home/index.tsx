...
export default function HomeContainer({response, redirectUrl, isAuthorized}: {response: any, redirectUrl: string, isAuthorized: boolean}) {
  ...
  return (
      ...
      <aside id="logo-sidebar" className={isShowSidebar ? "w-3/4 fixed z-50 shadow-md h-full" : "fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700 shadow-md"} aria-label="Sidebar">
      ...
      <div className="p-4 md:ml-64 mt-16">
        <h1>Welcome Lorem ipsum dolor sit, amet consectetur adipisicing elit. Saepe repudiandae officiis provident. Repudiandae nesciunt commodi nemo numquam eaque doloremque cum ipsum consequatur ab maiores, blanditiis reprehenderit velit delectus obcaecati ullam odio aliquam itaque doloribus! Laborum officia necessitatibus reiciendis natus facilis ab earum culpa recusandae doloribus? Alias quasi rerum saepe nostrum amet velit ea consectetur obcaecati? Repellat aspernatur voluptates maxime, eum, architecto, repellendus unde vero tempore assumenda quaerat vel odio! Voluptates itaque repudiandae tenetur ipsa consequatur, aperiam nemo incidunt autem similique saepe atque qui, rem harum, cum voluptatum recusandae commodi nihil alias animi laudantium provident debitis consequuntur magnam placeat? Odit, aliquid?</h1>
      </div>
      {isShowSidebar && <div drawer-backdrop="true" onClick={handleToggleSidebar} className="bg-main bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-30 h-full w-full" role='dialog'></div>}
    </section>
  )
}
